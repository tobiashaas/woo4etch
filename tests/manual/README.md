...
| `demo-mu/cart-bridge.php` | **Prototype** of the "cart as Etch dynamic data" bridge: hooks `etch/dynamic_data/option` to expose the cart as `{options.cart_items}` / `{options.cart_total}` (with mock rows when no cart is loaded, for builder preview). Lets the cart **items** be built as a pure Etch loop. See template 15 → "cart as pure Etch HTML". |
