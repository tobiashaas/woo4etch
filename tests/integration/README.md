...
| `07-store-api-checkout.php` | 5 | The A+ checkout bridge: the routes the frontend module writes to, the Germanized guard pieces, and the gateway allowlist wiring. Plus the gateway surface an adapter builds on — every method exposes `has_fields`/`store_api` as bools, `store_api` agrees with the allowlist for every method, and the `data-w4e-payment-fields` marker renders without fatal and leaves the region addressable (an unknown gateway id renders empty rather than failing). Enables cash-on-delivery and loads a cart so those assertions actually run instead of skipping; both restored at the end. |
