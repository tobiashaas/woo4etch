    ...
    public static function catalog() {
        return [
            ...
            'checkout' => [
                ...
                'limits'      => 'Two things to check before you install it. (1) Gateways: as shipped it offers redirect/offline ones (see store_api_checkout_gateways). A gateway that collects card data in the page — Stripe Elements, PayPal\'s inline buttons — needs its own fields and client JS; the plugin provides the seam for that (a data-w4e-payment-fields region and payment_data on the order POST) but ships no adapter, so out of the box those still fall back to the classic submit. If you do not want to write one, use [woo_checkout_block] or Woo\'s own checkout. (2) The legal checkboxes loop is fed by Germanized; without that plugin {options.checkout.checkboxes} is empty, so a terms-and-conditions checkbox has to be hand-built and validated yourself. Also note the form is billing-only: there is no separate shipping address and no ship_to_different_address toggle, so orders always ship to the billing address.',
                'area'        => 'Checkout',
            ],
            'mini-cart' => [
