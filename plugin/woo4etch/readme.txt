...
Each layout also states where it stops. These are complete layouts for a straightforward store, and every one of them leaves out something WooCommerce's own template renders — silently, with no error to notice it by. The Layouts tab prints a "Don't reach for this when" line under each entry: the checkout offers redirect/offline gateways only and carries no state/province field (so countries where WooCommerce requires one need extra PHP); the thank-you layout fires neither woocommerce_thankyou_{payment_method} nor woocommerce_thankyou, so offline gateways' payment instructions render nothing until you add [do_action] for both; the cart summary has no shipping row; the My Account orders list has no pagination and no Pay/Cancel row actions. Transactional emails have no layout by design — WooCommerce renders those from PHP templates.
...
= Unreleased =
* New: every ready-made layout now states where it stops. The Layouts tab prints a "Don't reach for this when" line under each entry, repeated at the top of the matching template doc: checkout — redirect/offline gateways only, no state/province field and no states bridge key (so AU, US, CA, ES, IN, JP … fail validation on both the classic and the Store API path), Germanized needed for legal checkboxes; thank-you — fires neither woocommerce_thankyou_{payment_method} nor woocommerce_thankyou, so offline gateways' payment instructions render nothing; cart — no shipping row between subtotal and total; My Account — orders list capped with no pagination and no Pay/Cancel row actions, Etch dashboard replaces Woo's; archives — no sorting control or result count; single product — buy box only (no reviews, tabs, related); mini-cart — read-only, CSS-hover reveal; notices — printing clears the queue. Transactional emails have no layout by design, and the docs now say so.
* New: {options.order.id} and {options.order.payment_method_id} — the keys needed to restore the thank-you hooks from the layout: [do_action hook="woocommerce_thankyou_{options.order.payment_method_id}" args="{options.order.id}"] and [do_action hook="woocommerce_thankyou" args="{options.order.id}"]. The previously documented {this.id} is the checkout PAGE's id on that endpoint, not the order's.
