...
IMPORTANT — updating the plugin does NOT update a layout that is already on your page. The push route is append-only and refuses a target that already carries the layout; that is what stops it from overwriting your work in the builder, and it also means a fix shipped inside a layout only reaches new installs. The Layouts tab flags an installed copy that predates a fix and names what it is missing. To take the update: open the page or template in the Etch builder, delete the layout's top-level section, then press "Add to page/template" again. Existing style records with the same selector are reused, never overwritten, so your styling survives.
