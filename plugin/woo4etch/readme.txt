...
* New: "Woo Notices" as a real Etch component — the admin page can create the notices region as an Etch component via Etch's public scripting API. The API only runs inside the builder, so it's a handshake: opt in, open the builder once, done — the admin page then shows the component id. Idempotent and guarded; on older Etch without the public API nothing happens and the page keeps waiting.
