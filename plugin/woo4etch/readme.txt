...
Each layout also states where it stops. These are complete layouts for a straightforward store, and every one of them leaves out something WooCommerce's own template renders — silently, with no error to notice it by. The Layouts tab prints a "Don't reach for this when" line under each entry: the checkout offers redirect/offline gateways only and is billing-only (no separate shipping address); the thank-you layout suppresses Woo's own order-details table, so its customer block is not shown; the cart has no shipping calculator; the My Account orders list has no pagination and no Pay/Cancel row actions. Transactional emails have no layout by design — WooCommerce renders those from PHP templates.

Updating an installed layout: a plugin update never silently rewrites blocks that are already on your page — the push route is append-only by design. Since 1.10.0 the plugin records what it installed, so the Layouts tab can tell the cases apart. An untouched copy gets a one-click "Update layout" button. A copy you have edited is left alone and gets manual steps instead, because replacing it would throw that work away: open the page or template in the Etch builder, delete the layout's top-level section, then press "Add to page/template" again. Either way, everything you added around the layout stays and existing style records are reused, never overwritten. Copies installed before 1.10.0 have no record and always use the manual route.
...
* Fix: the cart summary went subtotal → total with no shipping row, so a flat rate or a free-shipping threshold showed up only as an unexplained difference between two numbers. It now discloses shipping, gated the way Woo's own cart-totals template gates it — the amount via {options.cart_show_shipping}, or "Calculated at checkout" via {options.cart_shipping_notice} for exactly the case where "hide shipping costs until an address is entered" holds and there is none.
* New: the Layouts tab warns when an installed layout predates a fix and says what it is missing, with the delete-and-re-add instruction. Filter: woo4etch/layout_revisions.
* New: [do_action] gained skip_defaults="yes" and the hook markers gained data-w4e-args. The marker form could pass no arguments at all, which made it unusable for every hook that declares one — and firing woocommerce_thankyou bare is an ArgumentCountError on PHP 8, not a quiet no-op. Both forms now take positional args (numeric values as ints) and can suppress WooCommerce core's own callbacks for a hook (woo4etch/hook_core_defaults); third-party callbacks are never touched.
...
* New: installed layouts can be updated in place when the plugin can prove it is safe. Until now a fix shipped inside a layout only reached new installs, because the push route is append-only — which is what protects your builder work, and also what left existing sites on the old blocks. The plugin now records what it installed (a hash of the appended blocks) and the Layouts tab tells the cases apart: an untouched copy gets a one-click "Update layout" (only the layout's own blocks are replaced; everything around it and every style record stays), while a copy you have edited is left alone with the manual delete-and-re-add steps, because replacing it would throw that work away. Copies installed before 1.10.0 have no record and use the manual route. Before writing, the update also verifies that re-serializing the whole post reproduces it byte for byte, and refuses rather than risk content it does not own.
* Changed: the Shortcodes tab now leads with the shortcodes that exist because WooCommerce has to render them, and folds the rest away. Catalog entries gained an optional "prefer" key naming the Etch dynamic key that does the same job; the 21 entries carrying one moved into a collapsed "Data shortcodes" block that names that key, and WooCommerce's own 10 core tags into a reference block of their own. Nothing is deprecated or removed — those shortcodes work unchanged and remain the answer wherever dynamic keys do not reach (a widget, an email, a non-Etch template). The visible table went from 65 rows to 34.
* Removed: the experimental {woo.*} dynamic-data root. It re-mapped the same {options.*} builders under different names, and {options.*} was always the documented, guaranteed spelling. BREAKING for layouts using {woo.*} keys — the migration is mechanical ({woo.cart.items} -> {options.cart_items}, {woo.order} -> {options.order}, ...); the full table is in templates/15-woo4etch-plugin.md. The woo4etch/enable_woo_root and woo4etch/woo_root_data filters go with it.
* Docs: Etch compatibility re-verified against Etch 1.6.7 — the 1.4.20 floor still holds and every seam Woo4Etch uses is unchanged across that range.
