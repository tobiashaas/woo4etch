...
- **`{this.gallery_images}`** is a ready-to-loop array of image objects (`id, url, alt, title, caption, description, filename, sizes, srcset, width, height, filesize, mime_type`), added by Etch's WooCommerce integration (commit ETC-800) for `product` posts. **Ships in Etch 1.4.20+ — NOT in 1.4.19.** It **excludes the featured image by design** — render the featured image separately, then loop the gallery. The raw `_product_image_gallery` meta is just a CSV of IDs and is not usable directly.
