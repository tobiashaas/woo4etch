...
- **"Woo Notices" as a real Etch component** — new section on the admin page installs the notices region as an Etch component with one click, fully server-side: a `wp_block` post with the serialized block tree plus Etch's `etch_component_html_key`/`etch_component_properties` meta, styles merged into the style system like the pattern installer (same storage mechanism the Bricks2Etch migrator uses in production for its bundled components). Idempotent — reinstalling updates the existing component (found by key `WooNotices`) in place. Insert instances from the builder's component library and optionally swap the inline notices blocks in the layouts for instances. A *supported* server-side install API remains on the Etch wishlist (`ETCH-FEATURE-REQUESTS.md`).

### Added (admin QoL)

- **Page health check** — the admin page now resolves WooCommerce's page assignments (cart/checkout/account) and verifies the expected elements actually exist there: the area layout and the notices region, searched in the page content **and** in Etch templates. Missing pieces get an **Insert** button that appends the blocks directly to the assigned page (append-only, styles merged like the pattern installer) — no pattern-library detour.
- **Pattern freshness indicator** — the ready-made layouts table now shows *Up to date* / *Update available* per installed pattern (install stores a build fingerprint; layouts installed before this show *version unknown* until reinstalled once), plus an explicit note on what Reinstall touches: only the library pattern — inserted copies and existing class styles are never overwritten.

### Fixed

- **Notices region no longer reserves space when there is nothing to show** — the `.w4e-notices` wrapper hides itself while empty (`:has()`-based rule shipped with the styles); in the builder canvas `[woo_notices format="plain"]` renders a sample notice instead so the region stays visible and styleable while designing.
- **Cart cross-sells: fallback + hidden when empty.** WooCommerce only fills "You may also like" from each product's *Linked Products → Cross-sells* — unmaintained shops got an empty section with a stray heading. Now: with no cross-sells maintained, random visible catalog products (cart items excluded) fill the section (`woo4etch/cross_sells_fallback` filter to disable, `woo4etch/cross_sells_limit` for the count), and the section itself sits in an `options.cross_sells` condition so it disappears entirely when the list is empty.
