...
- **The shop and category layouts now ship pagination.** Neither archive layout contained `[woo_pagination]` (or any pagination) — with more products than one page holds, everything past page 1 was silently unreachable from the ready-made layouts. Both now render `[woo_pagination]` under the product grid (`.w4e-shop-pagination`: pill-style page links on Woo's native markup, ACSS-tokenized, hidden when there is only one page). Existing installs: re-push the layout (Layouts tab) to get it. Together with the per-page sync below, the page count always matches what the grid shows.
- **`[woo_pagination]` and the Etch product loop no longer disagree on the page count** (reported by Piotr). WooCommerce's products-per-page (`loop_shop_per_page`, default columns × rows = 16) applies to the **main query only** — Etch's main-query loop re-runs the request as a secondary query and fell back to the blog reading setting (10). `[woo_pagination]` counts pages from the main query, so the loop showed 10 products per page while pagination offered ceil(N/16) pages: products past the last link were unreachable. The plugin now syncs the main query's per-page onto secondary product loops on shop/taxonomy archives — same seam as the 1.6.0 archive-filter re-application; loops that set an explicit page size are left alone. The previously required workaround (a `loop_shop_per_page` filter forcing the main query down to the loop's 10) is no longer needed — `loop_shop_per_page` now controls grid and pagination together. Disable: `add_filter('woo4etch/sync_secondary_per_page', '__return_false');`. New integration check (`08-secondary-per-page.php`).
...
- **WooCommerce templates appear inside Etch's template hub.** Etch's hub UI renders only slugs from its own catalog — WooCommerce's registered types (`order-confirmation`, `product-search-results`, `coming-soon`) were in the REST list but never displayed anywhere in the builder, so the only way in was the WP Site Editor. Woo4Etch now injects a "WooCommerce" group into the hub (builder shell only, admins only) that is **pixel-native**: navigation header, content column and template cards are built by cloning Etch's own DOM nodes — Svelte's scoped-style hash classes ride along on clones, so the native tiles, spacing and hover-reveal apply as-is, while `cloneNode()` drops the original listeners and the buttons are rewired to Etch's own `?etch=magic&post_id=` deep links. A type that has no `wp_template` post yet is created on click (nonce-protected endpoint; materializes server-side, then lands in the builder — and opens the existing template rather than erroring when something created it meanwhile, which Etch's own `ensure_templates_saved()` does on every hub render). Page frames (`page-cart`, `page-checkout`) stay out of the group by design — opt one in via the `woo4etch/wc_templates` filter. Defensively built (idempotent, silently absent if Etch's markup changes) and removable the moment Etch lists unknown slugs natively (feature request updated). Filters: `woo4etch/etch_hub_templates`, `woo4etch/wc_templates`.
...
- **Admin page reorganized into four tabs** — Overview (shop status), Layouts, Settings, Shortcodes. The page had grown into one ~10-screen scroll stacking settings, layouts, health check and a 65-shortcode reference; each concern now gets its own screen (WP-native `nav-tab-wrapper`, no JS framework). Along the way: page slug is now `woo4etch` (old `woo4etch-shortcodes` bookmarks redirect, tab preserved), the H1 no longer claims to be a shortcode list, action notices render centrally on every tab, the health check's "Add layout" uses the same push route as the Layouts tab (one code path — `insert_into_page` remains for the notices region only), and the long settings explanations are folded behind one-sentence summaries (`<details>`).
- **Settings descriptions corrected**: the Store API toggle now says it also drives the ready-made checkout's interactions (shipping selection, place order — turning it off returns the checkout to classic submits); the checkout rate-limit description clarifies it protects the classic path only, since Store API orders are covered by WooCommerce's own rate limiting.
...
- **Saving a shipped layout in the builder destroyed its conditions.** The layout builder wrote a human-readable label into the condition's `conditionString` ("Has items"), while Etch re-derives the condition object *from that string* on every save — so the first builder save turned every generated condition into a comparison against a nonexistent path: the checkout page rendered blank, the account layout lost its login gate. Conditions now carry real expressions (`!options.cart_is_empty`, `pm.selected`) and the label moved to `metadata.name` where Etch expects it. A fast-check rejects builder-unsafe condition strings, `tools/repair-condition-strings.php` repairs affected pages/templates in place, and a full open-and-save round-trip on cart + checkout was verified byte-for-byte on a live site.
...
- **The shop status listed the checkout as "missing" on every site.** `Woo4Etch_Health::targets()` still carried the pre-1.9 assumption that no Woo4Etch checkout layout exists (`'layout' => ''`) and only looked for Woo's own shortcode/block — so a page carrying the ready-made A+ checkout was reported as missing, with no button to act on it. The checkout target now knows the layout (push button like the other areas) and accepts `w4e-checkout-form` as a marker.
