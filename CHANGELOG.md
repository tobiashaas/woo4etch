...
- **WooCommerce templates appear inside Etch's template hub.** Etch's hub UI renders only slugs from its own catalog — existing WooCommerce templates (`page-cart`, `page-checkout`, `order-confirmation`, `product-search-results`, `coming-soon`) were in the REST list but never displayed anywhere in the builder. Woo4Etch now injects a "WooCommerce" group into the hub (builder shell only, admins only) that is **pixel-native**: navigation header, content column and template cards are built by cloning Etch's own DOM nodes — Svelte's scoped-style hash classes ride along on clones, so the native tiles, spacing and hover-reveal apply as-is, while `cloneNode()` drops the original listeners and the Edit buttons are rewired to Etch's own `?etch=magic&post_id=` deep links. Defensively built (idempotent, silently absent if Etch's markup changes) and removable the moment Etch lists unknown slugs natively (feature request updated). Filter: `woo4etch/etch_hub_templates`.
...
- **Admin page reorganized into four tabs** — Overview (shop status), Layouts, Settings, Shortcodes. The page had grown into one ~10-screen scroll stacking settings, layouts, health check, template bridge and a 65-shortcode reference; each concern now gets its own screen (WP-native `nav-tab-wrapper`, no JS framework). Along the way: page slug is now `woo4etch` (old `woo4etch-shortcodes` bookmarks redirect, tab preserved), the H1 no longer claims to be a shortcode list, action notices render centrally on every tab, the health check's "Add layout" now uses the same push route as the Layouts tab (one code path — `insert_into_page` remains for the notices region only), and the long settings explanations are folded behind one-sentence summaries (`<details>`).
- **WooCommerce templates are managed only where they're edited — in Etch.** The wp-admin table is gone; the hub group took over its job completely: it now also lists template types that have no `wp_template` post yet and creates them on click (nonce-protected endpoint that materializes server-side and lands in the builder — and opens the existing one instead of erroring if something created it in the meantime, which Etch's own `ensure_templates_saved()` does on every hub render). One place, no wp-admin twin.
- **Stale admin texts corrected**: the Store API toggle now says it also drives the ready-made checkout's interactions (shipping selection, place order — turning it off returns the checkout to classic submits); the checkout rate-limit description clarifies it protects the classic path only, since Store API orders are covered by WooCommerce's own rate limiting.

### Fixed

- **The shop status listed the checkout as "missing" on every site.** `Woo4Etch_Health::targets()` still carried the pre-1.9 assumption that there is no Woo4Etch checkout layout (`'layout' => ''`) and only looked for Woo's own shortcode/block — so a page carrying the ready-made A+ checkout was reported as missing with no way to act on it. The checkout target now knows the layout (push button like the other areas) and accepts `w4e-checkout-form` as a marker.
