...
> **Updating a layout you already installed.** A plugin update never silently rewrites blocks already on your page — the push route is append-only by design. Since 1.10.0 the plugin records what it installed, so the Layouts tab can tell an **untouched** copy (which gets a one-click **Update layout** button) from one you have **edited** (which is left alone, because replacing it would throw that work away). In both cases everything you added around the layout stays, and style records are reused, never overwritten. See [Updating a layout you already installed](./templates/15-woo4etch-plugin.md#updating-a-layout-you-already-installed).
