...
- **Component install API:** components are `wp_block` posts plus `etch_component_*` meta and there are CRUD REST routes (`classes/RestApi/Routes/ComponentsRoutes.php`) — but no import/install mechanism for a packaged set. A documented "install component package" entry point would let companion plugins ship one-click demo layouts instead of copy-paste JSON. Concrete case: Woo4Etch's notices region (`[woo_notices format="plain"]` in a styled wrapper) is needed in every page layout (cart, single product, account, …) and is exactly a component — today each layout inlines a structurally identical copy and the docs tell users to "save as component" manually, because a plugin cannot install one.
