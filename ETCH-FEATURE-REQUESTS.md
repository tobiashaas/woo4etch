...
- **Server-side component install:** Etch's public scripting API now covers component CRUD (`etch.components.createAsync()/updateAsync({key, description, properties, blocks})` — docs.etchwp.com/public-api/components), which is great — but the runtime only exists inside the builder (`window.etch` on `?etch=magic` pages). A *server-side* equivalent (PHP function or documented REST payload) is still missing, so a plugin can't install a component at activation/install time. Concrete case: Woo4Etch's notices region (`[woo_notices format="plain"]` in a styled wrapper) belongs in every page layout as a component — Woo4Etch now creates it through the public API via a deferred handshake (admin opts in → a script on the next builder load creates it and reports the id back), which works but adds a "visit the builder once" step that a server-side entry point would remove.
