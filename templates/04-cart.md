...
- **Deleting the `page-cart` template doesn't remove it** — WooCommerce injects `page-cart` into the template hierarchy for the assigned cart page (any slug) and, without a DB template, serves its own plugin default: generic header/footer *template parts* instead of your Etch frame. If you don't want a cart-specific frame, keep `page-cart` as a thin clone of your generic `page` template (same header + `<main>` + `post-content` + footer) rather than deleting it. Same applies to `page-checkout`.
