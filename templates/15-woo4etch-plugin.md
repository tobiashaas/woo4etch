...
**WooCommerce templates in Etch** (same admin page): WooCommerce *registers* template types like `page-cart`, `page-checkout`, `order-confirmation`, `product-search-results` and `coming-soon` — but Etch's "new template" picker only knows the standard WordPress hierarchy, so until a `wp_template` post exists these are reachable only through the WP Site Editor. The table shows each one and materializes it with a click ("Make editable in Etch"): frames (cart/checkout) start as a clone of your generic `page` template so your house frame applies, everything else starts from WooCommerce's default content. Existing ones deep-link into the editor. Heads-up: these templates can't be removed by deleting them — WooCommerce backfills its plugin default (generic header/footer template parts) instead; keep them as thin clones.
